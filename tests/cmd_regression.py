...
# if castasrophic failure
test.quiet = False
for key in test.hspf_data_collection.keys():
    (operation, activity, opn_id, tcode) = key.split("_")
    if not test.should_compare(operation, activity, opn_id, tcode):
        continue
    hspf_data = test.hspf_data_collection[key]
    for cons in hspf_data.output_dictionary[key]:
        params = (operation, activity, opn_id, cons, tcode)
        test.check_con(params = params)
