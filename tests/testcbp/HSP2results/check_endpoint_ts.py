...
precip_ts = ModelLinkage(
    "PRCP",
    rchres1,
    {"right_path": "/TIMESERIES/TS039", "SVOLNO": "TS039", "link_type": 3},
)
ts1 = (
    precip_ts.read_ts()
...
exdd[(precip_ts.SVOL, precip_ts.SVOLNO)].append(
    pd.DataFrame(
        {
            "SVOL": precip_ts.SVOLNO,
            "SVOLNO": precip_ts.SVOLNO,
            "MFACTOR": precip_ts.MFACTOR,
            "TMEMN": precip_ts.TMEMN,
            "TMEMSB": precip_ts.TMEMSB,
        }
    )
)
exdd[(precip_ts.SVOL, precip_ts.SVOLNO)].append(
...
for row in exdd:
    data_frame = precip_ts.io_manager.read_ts(
...
precip_ts.io_manager.read_ts(
    category=Category.INPUTS, operation=None, segment=row.SVOLNO
)
