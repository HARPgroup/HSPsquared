...
ERRMSGS = (
    ...
    "HYDR: Solve converged to point outside valid range",
)  # ERRMSG4
...
def _hydr_(
    ...
    if nodfv:  # simple interpolation, the hard way!!
        ...
        v2 = volumeFT[indx + 1]
        rod1, od1[:] = demand(
            v1, rowsFT[indx, :], funct, nexits, delts, convf, colind, outdgt
        )
        rod2, od2[:] = demand(
            v2, rowsFT[indx + 1, :], funct, nexits, delts, convf, colind, outdgt
        )
        a1 = (v2 - vol) / (v2 - v1)
        o[:] = a1 * od1[:] + (1.0 - a1) * od2[:]
        ...
        ro, o[:] = demand(
            vol, rowsFT[indx, :], funct, nexits, delts, convf, colind, outdgt
        )  # $1159-1160

    # back to PHYDR
    if AUX1FG >= 1:
        dep, stage, sarea, avdep, twid, hrad = auxil(
            volumeFT, depthFT, sareaFT, indx, vol, length, stcor, AUX1FG, errors
        )  # initial

    # hydr-irrig
    irexit = (
        int(ui["IREXIT"]) - 1
    )  # irexit - exit number for irrigation withdrawals, 0 based ???
    # if irexit >= 1:
    irminv = ui["IRMINV"]
    rirwdl = 0.0
    # rirdem = 0.0
    ...
    for step in range(steps):
        ...
        if irexit >= 0:  # irrigation exit is set, zero based number
            if rirwdl > 0.0:  # equivalent to OVOL for the irrigation exit
                ...
                vv1 = volumeFT[indx]
                rod1, od1[:] = demand(
                    vv1, rowsFT[indx, :], funct, nexits, delts, convf, colind, outdgt
                )
                vv2 = volumeFT[indx + 1]
                rod2, od2[:] = demand(
                    vv2,
                    ...
                    while move != 0:
                        ...
                        if vol > vv2:
                            if indx >= nrows - 2:
                                if vol > topvolume:
                                    errors[1] += (
                                        1  # ERRMSG1: extrapolation of rchtab will take place
                                    )
                                move = 0
                            else:
                                move = 1
            ...
            if AUX3FG:
                if avdep > 0.0:
                    ...
                    if LKFG:  # flag, 1:lake, 0:stream
                        ustar = avvel / (
                            17.66 + (log10(avdep / (96.5 * DB50))) * 2.3 / AKAPPA
                        )
...
#######################################################################################
    ...
    convf = state["/TIMESERIES/" + domain + "/CONVF"][step]
    outdgt[:] = state["/TIMESERIES/" + domain + "/OUTDGT"][step, :]
    colind[:] = state["/TIMESERIES/" + domain + "/COLIND"][step, :]
    roseff = state["/STATE/" + domain + "/RO"]
    oseff[:] = state["/STATE/" + domain + "/O"][:]
    ...
    if volint <= 0.0:  #  case 3 -- no solution to simultaneous equations
        ...
        if nodfv:
            # ROUTE
            _hydr_route(domain, state, step)
        else:
            _hydr_noroute(domain, state, step)
...
def _hydr_aux(domain, state, step):
    # todo: integrate code with state
    """
    if vol >= topvolume:
        errors[1] += 1  # ERRMSG1: extrapolation of rchtab
    indx = fndrow(vol, volumeFT)
...
def _hydr_noroute(domain, state, step):
...
def _hydr_irr(domain, state, step):
   # todo: migrate this code to state system
   """
    if rirwdl > 0.0:  # equivalent to OVOL for the irrigation exit
        vol = irminv if irminv > vol - rirwdl else vol - rirwdl
        if vol >= volumeFT[-1]:
...
def init_hydr(domain, state):
    # todo: migrate this code to initialize the TIMESERIES & STATE from UCI or other
    """
    steps = int(ui["steps"])  # number of simulation steps
    delts = ui["delt"] * 60.0  # seconds in simulation interval
    uunits = ui["uunits"]
...
def _hydr_route(state, step):
    # todo: integrate this with state
    """
    rodz, odz[:] = demand(
        0.0,
        rowsFT[zeroindex, :],
