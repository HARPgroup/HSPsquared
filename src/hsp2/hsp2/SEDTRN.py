...
from hsp2.hsp2.state.state_fn import sedtrn_get_ix, sedtrn_init_ix, sedtrn_state_vars
...
def sedtrn(io_manager, siminfo, parameters, ts, state):
    ...
    # else:
    #     from hsp2.hsp2.state.state_fn_fn_defaults import state_step_hydr
...
def _sedtrn_(
    ...
    if abs(total_bedfr - 1.0) > 0.01:
        errorsV[6] += (
            1  # error message: sum of bed sediment fractions is not close enough to 1.0
        )
    ...
    for loop in range(simlen):
        ...
        if vol > 0.0:  # rchres contains water
            if (
                ...
                if sandfg == 1:  # case 1 toffaleti equation
                    ...
                    gsi, ferror, d50err, hrerr, velerr = colby(
                        avvele, db50m, hrade, fsl, tw
                    )
                    if ferror == 1:
                        ...
                        gsi = toffaleti(
                            avvele, db50e, hrade, slope, tw, wsande
                        )  # switch to toffaleti method
                    psand = (
                        gsi * twide * 10.5
                    ) / rom  # convert potential sand transport rate to conc in mg/l
                ...
                osed1 = (
                    sovol * sands + eovol * sand_ssed1
                )  # calculate amount of sand leaving through each exit gate in qty.vol/l.ivl
            ...
            scour = (
                -ised1 - (sands * svol)
            )  # calculate total amount of sand settling out during interval; this is equal to sand inflow + sand initially present
        ...
        osed4 += osed1
        sand_rssed1 = sand_t_rsed7 = (
            sand_rsed1 + sand_wt_rsed4
        ...
        osed4 += osed2
        silt_rssed2 = silt_t_rsed8 = (
            silt_rsed2 + silt_wt_rsed5
        )  # total storage in mg.vol/l
        ...
        osed4 += osed3
        clay_rssed3 = clay_t_rsed9 = (
            clay_rsed3 + clay_wt_rsed6
        )  # total storage in mg.vol/l
...
def colby(v, db50, fhrad, fsl, tempr):
    ...
    DF = array(
        [-999, 0.10, 0.20, 0.30, 0.60, 1.00, 2.00, 6.00, 10.00, 20.00, 1.0e2]
    )  # Depths for Figure 24
    CF = array(
        [-999, 0.00, 1.0e4, 5.0e4, 1.0e5, 1.5e5]
    )  # Concentrations of sediment for Figure 24
    P = array(
        [-999, 0.60, 0.90, 1.0, 1.0, 0.83, 0.60, 0.40, 0.25, 0.15, 0.09, 0.05]
    )  # Percentage Effect for Figure 24
    DP = array(
        [-999, 0.10, 0.15, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00]
    )  # Median diameters for Figure 24
    DG = array([-999, 0.10, 1.00, 10.0, 100.0])  # Depth values for Figure 26
    VG = array(
        [-999, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0, 8.0, 10.0]
    )  # Velocity values for Figure 26
    D50G = array(
        [-999, 0.10, 0.20, 0.30, 0.40, 0.60, 0.80]
    )  # Median values for figure 26
    TEMP = array(
        [-999, 32.0, 40.0, 50.0, 70.0, 80.0, 90.0, 100.0]
...
def toffaleti(v, fdiam, fhrad, slope, tempr, vset):
    ...
    gsm = (cmi * (fd11**zp) * (fd25 ** (oczm) - (fd11**oczm))) / (
        oczm * 1.0e30
    )  # middle layer transport capacity
    gsl = (cmi * ((fd11 ** (zn)) - ((2.0 * fdiam) ** (oczl)))) / (
        oczl * 1.0e30
    )  # lower layer transport capacity
    gsb = (cmi * ((2.0 * fdiam) ** (zn))) / 1.0e30  # bed layer transport capacity
